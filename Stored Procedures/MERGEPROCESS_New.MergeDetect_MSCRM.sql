...
CREATE PROC [MERGEPROCESS_New].[MergeDetect_MSCRM] --'lightning'
    @Client VARCHAR(100) 
AS
Declare @Date Date = (select cast(getdate() as date));
DECLARE @Account VARCHAR(100) = (SELECT CASE WHEN @client = 'lightning' THEN 'CRM_Account' ELSE CONCAT(@client,' PC_SFDC Account' ) END);--updateme
DECLARE @Contact VARCHAR(100) = (SELECT CASE WHEN @client = 'lightning' THEN 'CRM_Contact' ELSE CONCAT(@client,' PC_SFDC Account' ) END);--updateme
...
ON dm.PK_MergeID = fm.FK_MergeID AND ISNULL(fm.complete,0) = 0 AND fm.CreatedDate > GETDATE() - 14
